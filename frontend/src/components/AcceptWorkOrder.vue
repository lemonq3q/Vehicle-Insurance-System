<template>
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <div class="table_button_container">
              <el-button size="small" type="defualt" @click="handleDetail(scope.$index, scope.row)">详情</el-button>
              <el-button size="small" type="primary" @click="handleAccept(scope.$index, scope.row)">{{ scope.row.status == 1?'接单':'转移' }}</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
  ...
  <el-dialog v-model="dialog" title="变更处理人员">
    <el-form
      ...
      class="login_form"
    >
      <el-form-item prop="handleMerchantId" label="处理机构">
        <el-select
          style="width: 100%;"
          v-model="acceptOrderParams.handleMerchantId"
          filterable
          remote
          reserve-keyword
          placeholder="输入关键词选择处理机构"
          :remote-method="getAcceptMerchantOption"
          :loading="loading.accpetMerchant"
        >
          <el-option
            v-for="item in acceptMerchantOptions"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          />
...
const acceptMerchantOptions = ref([]);
...
const rules = reactive({
  handleMerchantId: [
    { required: true, message: '请选择接单机构', trigger: 'blur' },
  ],
  handleBy: [
...
const getUserOptionByMerchantId = async () => {
  ...
  if (acceptOrderParams.handleMerchantId == null || acceptOrderParams.handleMerchantId == undefined){
    ...
    acceptUserOptions.value = [];
    return;
  }
  await selectUserOptionByMerchantId(acceptOrderParams.handleMerchantId).then(res=>{
    res = res.data;
    if (res.code == 200){
...
const getAcceptMerchantOption = async (blurParam) => { 
  ...
  await selectUpstreamOption(blurParam).then(res=>{
    res = res.data;
    if(res.code == 200){
      acceptMerchantOptions.value = res.data.map(item => {
        return {
          label: `${item.code} ${item.name}`,
          value: item.id
        }
...
const handleAccept = (index, row) => {
  ...
  if (row.status == 1){
    acceptOrderParams.handleMerchantId = undefined;
    acceptOrderParams.handleBy = undefined;
    acceptOrderParams.status = 2;
  }
  else {
    acceptOrderParams.handleMerchantId = row.handleMerchantId;
    acceptOrderParams.handleBy = row.handleBy;
    acceptOrderParams.status = row.status;
    acceptMerchantOptions.value = [{
      label: `${row.handleMerchantCode} ${row.handleMerchantName}`,
      value: row.handleMerchantId
    }];
    getUserOptionByMerchantId();
  }
}

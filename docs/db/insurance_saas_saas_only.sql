...
CREATE TABLE `monitor_enterprise_daily_usage` (
  ...
  `workorder_count` bigint unsigned NOT NULL DEFAULT '0',
...
CREATE TABLE `monitor_monthly_revenue` (
  ...
  `recharge_amount` decimal(16,2) NOT NULL DEFAULT '0.00',
  `recharge_order_count` bigint unsigned NOT NULL DEFAULT '0',
...
CREATE TABLE `monitor_user` (
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `active_username` varchar(64) GENERATED ALWAYS AS ((case when (`deleted` = 0) then `username` else NULL end)) STORED,
  `active_phone` varchar(20) GENERATED ALWAYS AS ((case when (`deleted` = 0) then nullif(`phone`,_utf8mb4'') else NULL end)) STORED,

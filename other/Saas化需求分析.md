...
| 请求计数 | `RequestCountFilter`、`MaintenanceManager` | 当前活跃请求数统计，用于维护等待 | 不是企业用量统计，不能用于后台监控和套餐额度 |
...
企业拥有者可以购买系统套餐，按人数和时间付费，购买后创建订单。

功能需求：

1. 后台平台维护套餐，包括价格、人数、时长、OCR 额度、请求额度、上架状态。
2. 企业拥有者在门户或企业后台选择套餐并创建订单。
3. 订单创建时保存套餐快照，避免后续套餐价格变化影响历史订单。
4. 支付成功后创建或更新 `saas_subscription`。
5. 订阅生效后限制企业最大成员数、到期时间、OCR 额度和请求额度。
6. 订阅到期后企业状态可变为欠费限制，只允许续费和查看基础信息。
...
| `saas_plan`、`saas_order`、`saas_subscription` | 套餐与订阅 |
| `saas_usage_daily`、`saas_usage_monthly`、`saas_usage_archive_job` | 用量监控 |

### 3.6 用量统计与归档

请求量和 OCR 次数用于后台监控和套餐额度判断。考虑到实时写库成本高，先写 Redis，夜间归档到 MySQL。
...
目标是实现企业拥有者购买套餐，系统按订阅控制使用。

工作项：

1. 实现 `SaasPlanController`：套餐查询、后台增删改查。
2. 实现 `SaasOrderController`：创建订单、查询订单、取消订单。
3. 实现支付回调或模拟支付确认接口。
4. 支付成功后写 `saas_subscription` 和 `saas_subscription_change_log`。
5. 企业成员新增时检查 `user_limit`。
6. 登录或请求拦截时检查订阅状态和到期时间。
7. 到期企业进入欠费限制状态。
...
| 订单 | 新增企业侧模块 | `saas_order` | 拥有者下单 |
| 订阅 | 新增服务 | `saas_subscription` | 到期、人数、额度限制 |
...
4. 实现订单、订阅和企业限制。
5. 迁移商户模块，因为工单依赖商户。
6. 迁移文件和 OCR，因为工单编辑依赖附件和识别。
7. 迁移工单宽表到多表结构。
8. 迁移续保、导出、归档等依赖工单查询的功能。
9. 最后完善用量统计、后台监控和性能索引。

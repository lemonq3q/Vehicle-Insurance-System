 ...
 */
const empty = () => ({ code: '', name: '', description: '', billingCycle: 'YEAR', durationDays: 365, price: 0, listPrice: 0, memberLimit: 10, sortOrder: 10, status: 0, reason: '' });
...
export default {
   ...
   */
  data: () => ({
    form: empty(), errors: {}, submitting: false,
    numberFields: [
      { key: 'durationDays', label: '套餐时长（天）' }, { key: 'price', label: '销售价格（元）' },
      { key: 'listPrice', label: '划线价格（元）' }, { key: 'memberLimit', label: '成员上限（人）' },
      { key: 'sortOrder', label: '展示排序' }
    ]
  }),
  computed: {
    /**
   ...
   */
  mounted() {
    if (!this.isNew) planApi.detail(this.$route.params.id).then(data => { this.form = { ...data, reason: '' }; });
  },
  methods: {
    /**
    ...
    validate() {
      ...
      ['durationDays', 'memberLimit'].forEach(key => { if (Number(this.form[key]) <= 0) errors[key] = '必须大于 0'; });
      ['price', 'listPrice', 'sortOrder'].forEach(key => { if (Number(this.form[key]) < 0) errors[key] = '不能小于 0'; });
...
<style scoped>.plan-form{max-width:920px}.form-footer{display:flex;justify-content:flex-end;gap:8px;padding:16px 20px;border-top:1px solid var(--border)}.reason-field{margin-top:18px}</style>
